...
class VaccinePatientController extends Controller
{
    public function index(Request $request) {
        if($request->ajax()){
            ...
            return DataTables::of($data)
                    ...
                    ->addColumn('action', function($data){
                        return actionBtn('VaccinePatient #'. $data->name, route('healthcare_staff.vaccine_patient.index'), route('healthcare_staff.vaccine_patient.show', $data->id), route('healthcare_staff.vaccine_patient.edit', $data->id), route('healthcare_staff.vaccine_patient.destroy', $data->id));
                    })
                    ->rawColumns(['action'])
                    ->make(true);
        }
        ...
        return view('healthcare_staff.vaccine_patient.index', compact('data_vaccine_batch', 'data_patient'));
    }

    public function create(){
        return view('healthcare_staff.vaccine_patient.create');
    }
    ...
    public function show(VaccinePatient $vaccinePatient) {
        return view('healthcare_staff.vaccine_patient.show', ['data' => $vaccinePatient]);
    }

    public function edit(VaccinePatient $vaccinePatient) {
        $data_vaccine_batch = VaccineBatch::all();
        $data_patient = Patient::all();
        return view('healthcare_staff.vaccine_patient.edit', ['data' => $vaccinePatient, 'data_vaccine_batch' => $data_vaccine_batch, 'data_patient' => $data_patient]);
    }
    ...
    public function update(Request $request, VaccinePatient $vaccinePatient) {
        ...
        DB::beginTransaction();
        try{
            $vaccinePatient->vaccine_batch->update($request->all());
